...
Cloning
```console
gti clone https://github.com/jqueguiner/programming-language-detection.git programming-language-detection
```

Building Docker
```console
cd programming-language-detection && docker build -t programming-language-detection -f Dockerfile-api .
```

Running Docker
```console
echo "http://$(curl ifconfig.io):5000" && docker run -p 5000:5000 -d programming-language-detection
```

Calling the API for programming language detection
```console
curl -X POST "http://MY_SUPER_API_IP:5000/detect" -H "accept: application/json" -H "Content-Type: application/json" -d '{"url":"https://raw.githubusercontent.com/tensorflow/tensorflow/master/configure.py"}'
```
